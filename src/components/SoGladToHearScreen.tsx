...
interface SoGladToHearScreenProps {
  onBack: () => void;
  onNext: () => void;
}

export function SoGladToHearScreen({
  onBack,
  onNext
}: SoGladToHearScreenProps) {
  const { t } = useLanguage();

  useEffect(() => {
    // Send the new event for onboarding survey
    sendToFlutter(JSON.stringify({
      "event": "view_so_glad_to_hear",
      "eventProperties": {
      ...
      <div className="flex flex-col items-center flex-1 justify-center">
        ...
        <div className="text-center px-4 mb-8">
          <h1 className="font-normal text-gray-800 leading-tight" style={{ fontSize: 'min(6vw, 2rem)' }}>
            So glad to hear that! Let's keep building those positive habits together.
          </h1>
        </div>
