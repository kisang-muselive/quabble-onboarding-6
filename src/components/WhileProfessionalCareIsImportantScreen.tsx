...
interface WhileProfessionalCareIsImportantScreenProps {
  onBack: () => void;
  onNext: () => void;
}

export function WhileProfessionalCareIsImportantScreen({
  onBack,
  onNext
}: WhileProfessionalCareIsImportantScreenProps) {
  const { t } = useLanguage();

  useEffect(() => {
    // Send the new event for onboarding survey
    sendToFlutter(JSON.stringify({
      "event": "view_professional_care_important",
      "eventProperties": {
      ...
      <div className="flex flex-col items-center flex-1 justify-center">
        ...
        <div className="text-center px-4 mb-8">
          <h1 className="font-normal text-gray-800 leading-tight" style={{ fontSize: 'min(6vw, 2rem)' }}>
            While professional care is important, small daily practices like gratitude, breathing, and journaling can also play a powerful role in improving mental health.
          </h1>
        </div>
